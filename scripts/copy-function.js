...
async function copyFunctions() {
  console.log('Iniciando preparación de funciones para el build (versión genérica)...');
  ...
  if (await fs.pathExists(hostJsonSourcePath)) {
    ...
    console.warn('Advertencia: host.json no encontrado en la raíz. Asegúrate de que existe.');
  }

  // 2. Descubrir funciones automáticamente y procesarlas
  const functionFiles = await fs.readdir(sourceFunctionsDir);
  const functionsToProcess = functionFiles.filter(file => file.endsWith('.function.ts'));

  if (functionsToProcess.length === 0) {
    console.warn('No se encontraron archivos *.function.ts en src/users/functions/. No se crearon funciones.');
    return;
  }
  ...
  for (const functionFileName of functionsToProcess) {
    ...
    const compiledJsSourcePath = path.join(outputDir, 'src', 'users', 'functions', `${funcName}.function.js`);
    ...
    if (await fs.pathExists(compiledJsSourcePath)) {
      ...
      console.log(`Copiado ${funcName}.function.js a ${functionFolder}/index.js`);
    } else {
      console.error(`Error: Archivo compilado no encontrado para ${funcName} en ${compiledJsSourcePath}`);
      throw new Error(`Compilación fallida o archivo no encontrado para ${funcName}.`);
    }
    ...
    await fs.writeJson(path.join(functionFolder, 'function.json'), functionJson, { spaces: 2 });
    console.log(`function.json para ${funcName} creado en ${functionFolder}/function.json`);
  }
